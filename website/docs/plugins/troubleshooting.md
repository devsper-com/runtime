...
This page covers common issues encountered when developing and using devsper plugins, along with their solutions.

## Plugin Not Loading

### Entry Point Misconfigured

The most common cause of a plugin failing to load is an incorrect entry point in `pyproject.toml`. The entry point must be under the `devsper.plugins` group:
...
If your plugin module raises an `ImportError` at import time, devsper silently skips it by default. To surface these errors, enable debug mode:

```bash
DEVSPER_DEBUG=1 devsper doctor
```
...
Ensure the plugin is installed in the same Python environment as devsper:
...
Check your devsper configuration. If you use the `[tools]` section to restrict enabled categories, your plugin's tools must belong to an enabled category:
...
Use `devsper doctor` to list all registered tools and confirm your tools appear:
...
If a schema is invalid, devsper raises a validation error at registration time. Run with `DEVSPER_DEBUG=1` to see the full error detail.

## Version Conflicts

If your plugin depends on a library version that conflicts with devsper's own dependencies, pip will report a conflict during installation. To diagnose:
...
## Reading `devsper doctor` Output

`devsper doctor` prints a diagnostic report including:

- **Runtime version** -- the installed devsper version.
...
A plugin marked as `FAILED` in the output means it was discovered but could not be loaded. Re-run with `DEVSPER_DEBUG=1` for the full error.

## Debugging Tips

### Enable Debug Logging

Set the `DEVSPER_DEBUG` environment variable to get verbose output:
...
eps = entry_points(group="devsper.plugins")
for ep in eps:
...
| Error | Cause | Fix |
|-------|-------|-----|
| `No entry point found for 'my_plugin'` | Entry point name or group is wrong | Check `pyproject.toml` entry point group is `devsper.plugins` |
