...
This guide covers how to prepare, validate, and publish a devsper plugin to the registry.

## Prerequisites

Before publishing, you need:

1. A devsper registry account. Sign up at [registry.devsper.com](https://registry.devsper.com) using GitHub or Google OAuth.
2. The `devsper` CLI installed.
3. A Python build tool -- either `build` (`pip install build`) or `uv`.

## Plugin Structure

A publishable devsper plugin is a standard Python package with a `pyproject.toml` that declares a `devsper.plugins` entry point. The registry enforces the following requirements:

- Package name should follow the `devsper-plugin-*` naming convention
...
Repository = "https://github.com/you/devsper-plugin-example"

[project.entry-points."devsper.plugins"]
example = "devsper_plugin_example:register"
...
For CI environments, set the `DEVSPER_API_KEY` environment variable instead of using the interactive login flow.
...
- The `devsper.plugins` entry point is declared
...
1. Runs `devsper reg test` automatically
...
To publish a new version, bump the `version` field in your `pyproject.toml` and run `devsper reg publish` again. The registry does not allow overwriting an existing version -- you must increment the version number.
...
```bash
devsper reg yank devsper-plugin-example 0.1.0 --reason "security vulnerability"
```
...
Since devsper plugins are standard Python packages, you can also publish them to PyPI using [twine](https://twine.readthedocs.io/) or your preferred tool. Users can then install with plain `pip install`. The devsper registry is preferred for discoverability and ecosystem integration, but PyPI works as a complement -- especially for plugins that are also useful outside the devsper runtime.
...
- **Pin devsper compatibility.** Use the `requires-devsper` field in your pyproject metadata if your plugin depends on a specific runtime version.
- **Test before publishing.** Always run `devsper reg test` first.
