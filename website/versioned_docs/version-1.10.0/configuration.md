...
The knowledge graph is persisted under `data_dir/knowledge_graph.json`. Use `devsper doctor` to see node/edge counts and last updated time.
...
| `role` | string | `"hybrid"` | `"controller"`, `"worker"`, or `"hybrid"`. |
...
| `rpc_token` | string | (none) | Optional token for `/snapshot` and `/control`. |
| `max_workers_per_node` | int | 8 | Max concurrent tasks per worker node. |
| `node_tags` | list[string] | [] | Capability tags (e.g. `["gpu", "high-mem"]`). |
| `controller_url` | string | `"http://localhost:7700"` | Controller RPC URL. |
...
See [Distributed mode](/docs/distributed) for multi-node setup.
