---
sidebar_position: 0
title: Overview
description: devsper documentation — Distributed AI Swarm Runtime. Orchestrate multi-agent systems with a DAG-based swarm execution model. Get started, configure, and run with the CLI and SDK.
keywords:
  - devsper
  - documentation
  - AI swarm
  - multi-agent
  - distributed AI
  - getting started
---

import ThemedImage from '@theme/ThemedImage';

<div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
  <ThemedImage
    alt="devsper"
    sources={{
      light: '/img/logo.svg',
      dark: '/img/logo_dark.svg',
    }}
    width={120}
    height={140}
  />
</div>

# devsper

**Distributed AI Swarm Runtime**

Orchestrate multi-agent systems with a swarm execution model: tasks become a DAG, then run in parallel.

## Quick links

- [**Introduction**](/docs/introduction) What devsper is, core ideas, and how it works
- [**Configuration**](/docs/configuration) TOML config, env vars, and schema
- [**CLI**](/docs/cli) Commands, credentials, and workflows
- [**Examples**](/docs/examples) Example usage and patterns
- [**Distributed mode**](/docs/distributed) Multi-node (controller + workers) with Redis (v1.10)

Install from PyPI: **`pip install devsper`** · CLI: **`devsper`**
